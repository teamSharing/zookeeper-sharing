...
public class FastLeaderElection implements Election {
    ...
    private void sendNotifications() {
        for (long sid : self.getCurrentAndNextConfigVoters()) {
            ...
            ToSend notmsg = new ToSend(
                ...
                proposedLeader,//myid
                proposedZxid,//zxid
                logicalclock.get(),//原子类，可能会是leader周期自增
                QuorumPeer.ServerState.LOOKING,//looking状态
                sid,//就是配置server后的数字
                proposedEpoch,//leader周期
                qv.toString().getBytes(UTF_8));
    ...
    protected boolean totalOrderPredicate(long newId, long newZxid, long newEpoch, long curId, long curZxid, long curEpoch) {
         ...
         */
        //先比较epoch，第一次相等newEpoch > curEpoch)
        //                || ((newEpoch == curEpoch)这个地方是true，继续看后面
        //再比较zxid，如果zxid大于当前的直接成立
        //如果zxid等于，再比较myid，myid大的返回true
    ...
    private long getInitId() {
        //getVotingMembers这里有几种类型，只有非ObservingMembers才可以选举，不然这里就是false，下面的返回的就是负数，基本上不可能被选举了
    ...
    public Vote lookForLeader() throws InterruptedException {
        ...
        try {
            ...
            synchronized (this) {
                ...
                //更新了3个。myid、zxid，leader周期。实际上更新的还是自己，初始化
                ...
                if (n == null) {
                    ...
                    case LOOKING:
                        ...
                        if (voteSet.hasAllQuorums()) {
                             ...
                             */
                            if (n == null) {
                                //更新状态，如果myid是自己就改成leading，不然就是其他的2个
                                setPeerState(proposedLeader, voteSet);
                                Vote endVote = new Vote(proposedLeader, proposedZxid, logicalclock.get(), proposedEpoch);
                                leaveInstance(endVote);
                                //返回leader节点
