...
public class FollowerZooKeeperServer extends LearnerZooKeeperServer {
    ...
    protected void setupRequestProcessors() {
        //链路是2条。FollowerRequestProcessor->CommitProcessor->FinalRequestProcessor
    ...
    public void logRequest(TxnHeader hdr, Record txn, TxnDigest digest) {
        ...
        //顺便提一下，这里的类和之前看到的LeaderZooKeeperServer不是一个，且链条不一样。这里执行了SyncRequestProcessor，还有1个start，看run,会调用下个SendAckRequestProcessor
        //再提一下，LeaderZooKeeperServer的SyncRequestProcessor下一个是AckRequestProcessor。也就是之前1.3看到的给自己发ack。这里应该是2.2返回ack
