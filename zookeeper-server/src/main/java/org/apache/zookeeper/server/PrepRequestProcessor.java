    ...
    public void run() {
        ...
        try {
            while (true) {
                ...
                //拿队列，这个队列之前看到已经放过了->RequestThrottler.submitRequest()
    ...
    private void pRequestHelper(Request request) {
        try {
            ...
            case OpCode.create2:
                ...
                //zks.getNextZxid()->AtomicLong从0开始，这里比如第一次create就加1
                //这里再提下，long是64位。低32位是事务id，高32位代表了leader的唯一周期，所以即使leader挂了，低32位又从0开始算zxid也没事
                //follower连接上leader后，leader服务器会根据自己服务器上最后提交的zxid和follower上的zxid进行对比，要么回滚要么和leader同步
