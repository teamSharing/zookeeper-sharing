...
public abstract class ServerCnxnFactory {
    ...
    //1.1 最开始的地方，先看下服务端和客户端的连接，不然接收不到 create /666，这里的上面就是main启动
    public static ServerCnxnFactory createFactory() throws IOException {
        //默认以nio方式创建。可以指定vm option
        ...
        try {
            //这个实际上调用的是NettyServerCnxnFactory()构造
            //-Dzookeeper.serverCnxnFactory=org.apache.zookeeper.server.NettyServerCnxnFactory
            //这反射创建，然后NettyServerCnxnFactory.start绑定端口启动，bootstrap.bing(localAddress).
            //就是建立服务端和客户端的连接了，客户端的create /666服务端就可以接收,可以看这个channelRead方法
